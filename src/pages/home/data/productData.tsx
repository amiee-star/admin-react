...
import AddProduct from "@/components/modal/data/addProductInfo.modal"
...
import { Creators, proClassData, TradeClassList } from "@/interfaces/api.interface"
...
const productData = (props: PageProps) => {
				...
				return (
					<Space size={10}>
						...
						<Button
							...
							type="primary"
							danger
							onClick={downAway(item.id, "delete")}
							disabled={statResult(item.creatorName)}
						>
							删除
						</Button>
		...
		(type: string) => () => {
			...
			if (data.length == 0) {
				...
				Modal.confirm({
					...
					content: type == "delete" ? "是否删除所有勾选产品数据？" : "是否下架所有勾选产品数据？",
					closable: true,
					onOk: () => {
	...
	const downAway = (id: number, type: string) => () => {
		Modal.confirm({
			...
			content: type == "down" ? "是否下架该产品数据？" : "是否删除该产品数据？",
			closable: true,
			onOk: () => {
	...
	// 抛出事件
	useEffect(() => {
		eventBus.on("doProductData", page => setParams({ ...withParams.current, pageNum: page ? pageIndex.current : null }))
		return () => {
		...
		<Row className="data-form full">
			<Col className="form-search" span={24}>
				<Form form={form} onFinish={onFinish} onValuesChange={onValuesChange}>
					...
					>
						<Select placeholder={"一级分类"} allowClear>
					...
					<Form.Item name="solutionId" style={{ display: "inline-flex", width: "calc(10% - 4px)", marginLeft: 10 }}>
						{/* tradeClassTwo */}
					...
					<Form.Item
						...
						style={{ display: "inline-flex", width: "calc(20% - 4px)", marginLeft: 20 }}
					>
						<Select placeholder={"显示类型"} allowClear>
