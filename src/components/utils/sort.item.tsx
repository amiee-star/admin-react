...
interface Props {
	onChange?: (data: string[], e: Sortable.SortableEvent) => void
	dataIdAttr?: string
	handle?: string
...
const SortItem = React.forwardRef<Sortable, PropsWithChildren<Props>>((props, ref) => {
	const { onChange, dataIdAttr = "data-id", children, handle, direction, filter, onEnd: propsEnd } = props
	const sortTableRef = useRef<Sortable>()
	const sortBox = useRef<HTMLDivElement>(null)
	const onEnd = useCallback((event: Sortable.SortableEvent) => {
		if (event.newIndex !== event.oldIndex) {
			onChange && onChange(sortTableRef.current!.toArray(), event)
		}
		propsEnd && propsEnd(event)
	}, [])
	useEffect(() => {
