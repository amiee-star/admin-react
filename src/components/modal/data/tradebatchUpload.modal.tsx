...
const BatchUploadModal: React.FC<Props & ModalRef> = props => {
	...
	const uploadProps = {
		name: "file",
		action: `${urlFunc.requestHost()}/common/solution/importSolution`,
		headers: {
		...
		onChange(info: any) {
			if (info.file.status === "done") {
				if (info.file.response.code == 200) {
					message.success(`行业数据批量上传成功`)
				} else {
					message.error(info.file.response.msg || `行业数据批量上传失败`)
				}
					...
					>
						...
						<Form.Item label="模板下载：">
							<Button
								type="text"
								style={{ color: "#02A7F0" }}
								href={`${ProxyConfig.api.pro}/common/solution/downloadSolutionTemplate?token=${token}`}
							>
								行业批量上传模板
							</Button>
						</Form.Item>
